...
function arrayContainsOddNumbers(array) {
    ...
    return array.some(number => number % 2 === 1)
...
function getStringOfDivs(array) {
    let newArray = array.map( string => `<div>${string}</div>`);
...
function getStringOfElements(array, element) {
    let newArray = array.map( string => `<${element}>${string}</${element}>`)
    return newArray.join('');
...
function getMeanAverage(array) {
    let average = 0;
    array.forEach(number => average += number)
    if (array.length === 0) {
        return 0;
    }
